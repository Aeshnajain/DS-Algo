...
bool haspath(int src,int dest,vector<int>&visited)
{
    if(src==dest)
    {
        return true;
...
int allpaths(int src,int dest,vector<int>&visited,int tw,string ans)
{
    if(src==dest)
    {
        cout<<ans<<endl;
    ...
    for(int i=0;i<graph[src].size();i++)
    {
        ...
        if(!visited[e.v])
        {
            count+=allpaths(e.v,dest,visited,tw+e.w,ans+to_string(e.v)+" ");
        }
    }
    visited[src]=false;
